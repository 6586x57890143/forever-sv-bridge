...
#   pwsh forever_sv_bridge.tests.ps1                        same on Linux (systemctl shadowed instead of the task cmdlets)
...
# (scheduled tasks, systemctl, registry, sleep) is shadowed by a same-named function.
...
# ---- fake install ----------------------------------------------------------
$Temp       = (New-Item -ItemType Directory -Force -Path (Join-Path ([IO.Path]::GetTempPath()) ("svbridge_test_" + [guid]::NewGuid().ToString("N")))).FullName   # long form (CI TEMP is an 8.3 path)
$InstallDir = "$Temp/install"
$Log        = "$InstallDir/forever_sv_bridge.log"
Touch "$Temp/World of Warcraft/_classic_beta_/WowB.exe" ""
Use-GameDir "$Temp/World of Warcraft/_classic_beta_"                          # exists -> native separators from here on
$GamePath = $Root
...
Check "newest account wins" ((Get-AccountDir) -eq (Get-Item -LiteralPath $acct).FullName)
...
if ($Win) {
    ...
    ${env:ProgramFiles(x86)} = "$Temp/nope"; $env:ProgramFiles = "$Temp/nope"
} else {
    $pfx = "$Temp/pfx"                                                        # a wine prefix: product.db says C:/Games, c: -> drive_c
    function Get-WinePrefixes { $pfx }
    Touch "$pfx/drive_c/Games/World of Warcraft/_classic_beta_/WowB.exe" ""
    Touch "$pfx/drive_c/ProgramData/Battle.net/Agent/product.db" "junk`0C:/Games/World of Warcraft`0more"
    Check "found via wine product.db"  ((Find-BetaDir) -eq "$pfx/drive_c/Games/World of Warcraft/_classic_beta_")
    Remove-Item "$pfx/drive_c/ProgramData" -Recurse -Force
    Touch "$pfx/drive_c/Program Files (x86)/World of Warcraft/_classic_beta_/WowB.exe" ""
    Check "found via wine default dir" ((Find-BetaDir) -eq "$pfx/drive_c/Program Files (x86)/World of Warcraft/_classic_beta_")
    function Get-WinePrefixes { }
}
$script:asked = 0
...
Unmock Get-ItemProperty Read-Host Get-WinePrefixes

Write-Host "Install-Bridge / Uninstall-Bridge (task cmdlets / systemctl shadowed)"
$script:calls = @{}
...
function systemctl { $script:calls["systemctl " + ($args[1..2] -join " ")] = $args }
$env:XDG_CONFIG_HOME = "$Temp/cfg"
$Self = $script                                                           # run from the repo -> copies itself
Install-Bridge | Out-Null
$dst = Join-Path $InstallDir "forever_sv_bridge.ps1"
Check "script copied into install dir"    ([IO.File]::ReadAllBytes($dst).Length -eq [IO.File]::ReadAllBytes($script).Length)
...
if ($Win) {
    ...
    $cmd = $script:calls["New-ScheduledTaskAction"] -join " "
} else {
    Check "unit enabled and (re)started"      ($script:calls.ContainsKey("systemctl enable ForeverSVBridge") -and $script:calls.ContainsKey("systemctl restart ForeverSVBridge"))
    $cmd = Get-Content -Raw "$Temp/cfg/systemd/user/ForeverSVBridge.service"
    Check "unit runs pwsh at login"           ($cmd -match '(?m)^ExecStart="\S*pwsh" -NoProfile' -and $cmd -match "WantedBy=default.target")
}
Check "task points at copy + game path" ($cmd -like "*-File `"$dst`" -Watch -GamePath `"$Root`"*")
$GamePath = $null                                                         # no -GamePath -> asks Find-BetaDir
function Find-BetaDir { $script:calls["find"] = 1; $Root }
Install-Bridge | Out-Null
Check "falls back to Find-BetaDir"     ($script:calls["find"] -eq 1)

Uninstall-Bridge | Out-Null
if ($Win) { Check "uninstall stops and removes task" ($script:calls.ContainsKey("Stop-ScheduledTask") -and $script:calls.ContainsKey("Unregister-ScheduledTask")) }
else      { Check "uninstall disables and removes unit" ($script:calls.ContainsKey("systemctl disable --now") -and -not (Test-Path "$Temp/cfg/systemd/user/ForeverSVBridge.service")) }
